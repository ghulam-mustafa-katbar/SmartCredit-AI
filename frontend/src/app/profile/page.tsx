...
import { UploadCloud, ArrowRight, ShieldCheck, CheckCircle2, FileText, X, Image } from 'lucide-react';
...
export default function ProfileSubmissionPage() {
  ...
  const processFiles = (files: FileList | null) => {
    ...
    Array.from(files).forEach((file) => {
      if (!ACCEPTED_TYPES.includes(file.type)) {
        setUploadError(`"${file.name}" is not a supported type. Use PDF, JPG, PNG, or WEBP.`);
        return;
      }
      ...
      <div className="min-h-screen bg-[#0f172a] text-slate-50 flex flex-col items-center justify-center p-4">
        <div className="glass-card p-8 max-w-md w-full text-center">
          ...
          <p className="text-slate-400 mb-8">
            Our AI is now analyzing your alternative financial data. Your credit score will be updated shortly.
          </p>
          <Link
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-50 flex flex-col items-center py-12 px-4 relative font-sans">
      <div className="absolute top-0 left-0 w-full h-96 bg-blue-600/10 blur-[120px] rounded-full pointer-events-none -translate-y-1/2"></div>

      <div className="max-w-3xl w-full z-10">
        <Link href="/dashboard/customer" className="text-sm text-blue-400 hover:text-blue-300 mb-8 inline-block">
